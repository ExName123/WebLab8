...
function pageBtnHandler(event) {
    if (event.target.dataset.page) {
        ...
        downloadData(event.target.dataset.page, searchQuery);
        window.scrollTo(0, 0);
    }
}
...
function downloadData(page = 1, searchQuery = '') {
    ...
    // Добавляем параметр q к запросу
...
document.querySelector('#search-input').addEventListener('input', function () {
    ...
    fetch(`http://cat-facts-api.std-900.ist.mospolytech.ru/autocomplete?q=${query}`)
        ...
        .catch(error => {
            console.error('Ошибка при выполнении AJAX-запроса для автодополнения:', error);
        });
});
